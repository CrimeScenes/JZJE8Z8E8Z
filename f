...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = shared.Forbidden.Sentinel.CamLock.Normal.Radius_Visibility
            Circle.Radius = shared.Forbidden.Sentinel.CamLock.Normal.Radius
...
local function GetTarget()
    if shared.Forbidden.Sentinel.Silent.mode == "target" then
        return TargetPlayer  
    elseif shared.Forbidden.Sentinel.Silent.mode == "normal" then
        return ClosestPlrFromMouse()  
    end
end

Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == shared.Forbidden.Sentinel.Target.Keybind:lower() then
        if shared.Forbidden.Sentinel.CamLock.Normal.Enabled then
            if IsTargeting then
    ...
    elseif Key:lower() == shared.Forbidden.Sentinel.Target.UntargetKeybind:lower() then
        IsTargeting = false
        TargetPlayer = nil  
    end
end)
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        local BodyPart
        if shared.Forbidden.Sentinel.CamLock.Normal.ClosestPart then
            BodyPart = GetClosestBodyPart(TargetPlayer.Character)
        else
            BodyPart = TargetPlayer.Character:FindFirstChild(shared.Forbidden.Sentinel.CamLock.Normal.HitPart)
        end

        if BodyPart then
            local predictedPosition
            if shared.Forbidden.Sentinel.CamLock.Normal.Aligner then
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * shared.Forbidden.Sentinel.CamLock.Normal.Prediction)
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * shared.Forbidden.Sentinel.CamLock.Normal.Prediction)
            end
            
            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if shared.Forbidden.Sentinel.CamLock.Normal.SmoothnessEnabled then
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, shared.Forbidden.Sentinel.CamLock.Normal.Smoothness)
                else
                    Camera.CFrame = DesiredCFrame
                end
            end

            if shared.Forbidden.Sentinel.Silent.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                if shared.Forbidden.Sentinel.Silent.mode == "target" then
                    local closestPoint
                    if shared.Forbidden.Sentinel.Silent.TargetMode == "OptimalTargetPoint" then
                        closestPoint = GetOptimalTargetPoint(TargetPlayer.Character)
                    elseif shared.Forbidden.Sentinel.Silent.TargetMode == "Closest Point" then
                        closestPoint = GetClosestPoint(TargetPlayer.Character)
                    elseif shared.Forbidden.Sentinel.Silent.TargetMode == "BasicTargeting" then
                        closestPoint = GetClosestHitPoint(TargetPlayer.Character)
                    end

                    local velocity = GetVelocity(TargetPlayer, "Head")  
                    Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * shared.Forbidden.Sentinel.Silent.Prediction)
                elseif shared.Forbidden.Sentinel.Silent.mode == "normal" then
